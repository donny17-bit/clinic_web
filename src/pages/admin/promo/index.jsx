...
const Promo = ({ query, data }) => {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      ...
      await axios.delete(`promo/${idDelete}`, config);
  ...
  const refreshData = async () => {
    try {
      const result = await axios.get("promo");
  ...
  const handleEdit = (id) => {
    router.push(`/admin/promo/edit/${id}`);
  };
      ...
      <section className=" my-5 md:my-10 px-[10%]">
        ...
        <Flex direction={{ base: "column", md: "row" }} mt="20px">
          ...
          <Flex flex="4" direction="column">
            <Flex>
              <Link as={NextLink} href="/admin/promo/add-promo">
                <Button
                  bgColor="#b866cf"
                  _hover={{ bgColor: "#b866cf90" }}
                  leftIcon={<Icon as={BiAddToQueue} color="white" />}
          ...
          <Modal isOpen={isOpen} onClose={() => handleCloseModal()}>
            <ModalOverlay />
            <ModalContent>
              <ModalHeader>Hapus Promo</ModalHeader>
              <ModalCloseButton />
              <ModalBody>
                <Text>
                  Promo yang telah dihapus tidak dapat dikembalikan. Yakin ingin
                  menghapus promo?
                </Text>
              </ModalBody>
